...
#   ./stage1_install.sh --slack-channel "#my-channel" --repo-name "browser-automation"
#
# Arguments (optional — saved to .env for use by other scripts):
#   --slack-channel  Slack channel for Phantom to use (e.g. "#test_phantom2")
#   --repo-name      GitHub repository name (e.g. "browser-automation")
#
...
while [[ $# -gt 0 ]]; do
    case "$1" in
        ...
        --repo-name)
            ...
            ;;
        *)
            # Ignore unknown args (backward compat)
            shift
            ;;
    esac
done
...
# --- Write .env file (session config for other scripts) ----------------------
# Only write values that were provided via arguments.
# Existing values in .env are preserved if not overridden.

if [ -n "$SLACK_CHANNEL" ] || [ -n "$REPO_NAME" ]; then
    echo "▶ Writing session config to $ENV_FILE..."

    # Load existing .env if present (so we don't clobber values)
    EXISTING_SLACK=""
    EXISTING_REPO=""
    if [ -f "$ENV_FILE" ]; then
        EXISTING_SLACK=$(grep "^SLACK_CHANNEL=" "$ENV_FILE" 2>/dev/null | cut -d'=' -f2- | tr -d '"' || true)
        EXISTING_REPO=$(grep "^REPO_NAME=" "$ENV_FILE" 2>/dev/null | cut -d'=' -f2- | tr -d '"' || true)
    fi

    # Use provided values, fall back to existing
    FINAL_SLACK="${SLACK_CHANNEL:-$EXISTING_SLACK}"
    FINAL_REPO="${REPO_NAME:-$EXISTING_REPO}"

    # Write .env
    cat > "$ENV_FILE" <<ENVEOF
# Phantom session config — auto-generated by stage1_install.sh
# This file is read by git_auto_commit.sh and stage2_start.sh
SLACK_CHANNEL="${FINAL_SLACK}"
REPO_NAME="${FINAL_REPO}"
ENVEOF

    [ -n "$FINAL_SLACK" ] && echo "  ✓ SLACK_CHANNEL=$FINAL_SLACK"
    [ -n "$FINAL_REPO" ]  && echo "  ✓ REPO_NAME=$FINAL_REPO"
